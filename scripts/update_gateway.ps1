...
Write-Host "For API: $API_ID"
Write-Host "New Config ID: $CONFIG_ID"
Write-Host "=========================================="

# 1. Create a new API Config
Write-Host "`n[1/2] Creating new API Config..."
$createCmd = "gcloud api-gateway api-configs create $CONFIG_ID --api=$API_ID --openapi-spec=api_gateway_config.yaml --project=$PROJECT_ID --backend-auth-service-account=$SERVICE_ACCOUNT"
...
$updateCmd = "gcloud api-gateway gateways update $GATEWAY_ID --api=$API_ID --api-config=$CONFIG_ID --location=$REGION --project=$PROJECT_ID"
