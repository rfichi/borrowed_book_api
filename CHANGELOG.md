...
### Fixed
- **API Gateway**: Removed security restrictions from `/docs` and `/openapi.json` endpoints to fix 401 Unauthorized errors and allow public access to documentation.
...
### Changed
- **Git Workflow**: Updated `git-workflow-helper` and `command-execution-rules` skills to enforce `--no-pager` for git inspection commands (`log`, `show`, `diff`).
- **Safety**: Prevented command execution locking by suppressing interactive pagers.

## [0.5.6] - 2026-02-03

### Changed
- Updated `auto-pr.yaml` workflow to include `test/*` branches for automatic PR creation.
- Updated git workflow skills documentation.
