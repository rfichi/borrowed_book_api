...
def test_gateway():
    ...
    # 3. List Users
    print("3. List Users")
    resp = httpx.get(f"{GATEWAY_URL}/users", headers=get_headers(token))
    if resp.status_code == 200:
        ...
        if user_id:
            ...
            resp_u = httpx.get(
                f"{GATEWAY_URL}/users/{user_id}", headers=get_headers(token)
            )
    ...
    # 5. List Books
    print("5. List Books")
    resp = httpx.get(f"{GATEWAY_URL}/books", headers=get_headers(token))
    if resp.status_code == 200:
    ...
    resp = httpx.get(f"{GATEWAY_URL}/books/{book_id}", headers=get_headers(token))
    if resp.status_code == 200:
    ...
    resp = httpx.get(
        f"{GATEWAY_URL}/users/{user_id}/borrow-history", headers=get_headers(token)
    )
    if resp.status_code == 200:
        history = resp.json()
    ...
    print(
        f"   9.1 Borrowing book {book_id} with non-existent user {non_existent_user_id}"
    )
    resp = httpx.post(
        f"{GATEWAY_URL}/borrow/{book_id}/borrow",
    ...
    print(
        f"   9.3 Returning book {book_id} with non-existent user {non_existent_user_id}"
    )
    resp = httpx.post(
        f"{GATEWAY_URL}/borrow/{book_id}/return",
        json={"user_id": non_existent_user_id},
